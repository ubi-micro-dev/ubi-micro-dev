...
while read -r IMAGE; do
    ...
    # Use Python version instead of SBCL
    python report.py $(pwd)/_site/${IMG}.html grype/${IMG}.json trivy/${IMG}.json ${IMAGE} || true

    find _site -name \*.html
done

# Create the index using Python
python report.py make-index
